...
public class AboutDialog {
    ...
    public static void show(Activity activity, View.OnTouchListener logoOnTouchListener) {
        ...
        ImageView logoImageView = view.findViewById(R.id.imageView_about_dialog_logo);

        if (logoOnTouchListener != null) {
            logoImageView.setOnTouchListener(logoOnTouchListener);
        }

        view.findViewById(R.id.imageView_about_dialog_social_google_plus).setOnClickListener(
                v -> Navigator.toWebPage(activity,
                        "https://plus.google.com/communities/115317471515103046699"));
        view.findViewById(R.id.imageView_about_dialog_social_github).setOnClickListener(
                v -> Navigator.toWebPage(activity, "https://github.com/sorcererXW/SorceryIconPack")
        );
        view.findViewById(R.id.imageView_about_dialog_social_coolapk).setOnClickListener(
                v -> Navigator.toWebPage(activity,
                        "http://www.coolapk.com/apk/com.sorcerer.sorcery.iconpack")
        );
        TextView versionTextView = view.findViewById(R.id.textView_about_dialog_version);

        TextView creditsTextView = view.findViewById(R.id.textView_about_dialog_credits);
        TextView iconCountTextView = view.findViewById(R.id.textView_about_dialog_icon_count);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            creditsTextView.setText(
                    Html.fromHtml(htmlBuilder.toString(), Html.FROM_HTML_MODE_LEGACY));
        } else {
            creditsTextView.setText(Html.fromHtml(htmlBuilder.toString()));
        }
        creditsTextView.setMovementMethod(LinkMovementMethod.getInstance());
        MaterialDialog dialog = Dialogs.builder(activity).customView(view, true).build();
