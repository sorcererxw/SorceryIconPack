...
public class SettingsFragment extends PreferenceFragment {
    ...
    public void onAttach(Activity activity) {
        ...
        mActivity = activity;
        mPrefs = mActivity.getSharedPreferences(SHARED_PREFERENCE_NAME, MODE_WORLD_READABLE);
        mGlobalLoadActive = mPrefs.getBoolean("pref_global_load", false);
    }
    ...
    private boolean tryAndApplyIcon(final ApplicationInfo themePackage) {
        ...
        if (RootTools.isAccessGiven()) {
            ...
            if (!new File(mActivity.getCacheDir().getAbsolutePath() + "/icons").exists()) {
                try {
                    RootTools.getShell(true).add(new CommandCapture(0,
                            ...
                            "chmod 777 " + mActivity.getCacheDir().getAbsolutePath()
                                    + "/icons"));
                } catch
                        (Exception e22) {
                    e22.printStackTrace();
            ...
            if (!appIsInstalledInMountASEC()) {
                try {
                    Runtime.getRuntime()
                            .exec("rm " + mActivity.getExternalCacheDir().getAbsolutePath()
                                    + "/tmp.apk");
                } catch (Exception e) {
                    e.printStackTrace();
                }
