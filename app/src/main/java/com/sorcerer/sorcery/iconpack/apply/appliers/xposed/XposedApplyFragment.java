...
public class XposedApplyFragment extends PreferenceFragmentCompat {
    ...
    private void init() {
        ...
        mRefreshPreference.setOnPreferenceClickListener(
                preference -> {
        ...
        mClearNovaCachePreference.setOnPreferenceClickListener(
                preference -> {
                    RxSU.getInstance().su()
                            .filter(grant -> grant)
                            .subscribeOn(Schedulers.newThread())
        ...
        mSharedPreferences = RxSharedPreferences.create(
                getActivity().getSharedPreferences(XPOSED_PREFERENCE, MODE_WORLD_READABLE)
        );
