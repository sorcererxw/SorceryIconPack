...
import static com.sorcerer.sorcery.iconpack.apply.xposed.Constants.REPLACEMENT_DISPOSE_PREFERENCE_KEY;
...
public class XposedInstaller {
    ...
    static Observable<List<IconReplacement>> install(Context context) {
        ...
        return Observable.just(1)
                ...
                .map(iconReplacements -> {
                    context.getSharedPreferences(XPOSED_PREFERENCE, Context.MODE_PRIVATE).edit()
                            .putString(REPLACEMENT_DISPOSE_PREFERENCE_KEY,
                                    new Gson().toJson(iconReplacements))
                            .commit();
                    setXposedPrefWorldReadable(context);
                    return iconReplacements;
                });
    }

    @SuppressLint("SetWorldReadable")
    private static void setXposedPrefWorldReadable(Context context) {
        File prefsFile = new File(
                context.getFilesDir() + "/../shared_prefs/" + XPOSED_PREFERENCE + ".xml");
        File prefsFolder = new File(context.getFilesDir() + "/../shared_prefs");
        File dataFolder = new File(context.getFilesDir() + "/..");
        Stream.of(prefsFile, prefsFolder, dataFolder).filter(File::exists).forEach(file -> {
    ...
    private static void cacheDrawable(Context context, String packageName, int resId,
        ...
        FileOutputStream stream =
                context.openFileOutput(packageName + "_" + resId, Context.MODE_PRIVATE);
        stream.write(bos.toByteArray());
        stream.close();
        FileUtil.setWorldReadable(f);
    }

    public static void onPackageInstall(Context context, String packageName) {
        if (!context.getSharedPreferences(Constants.XPOSED_PREFERENCE, Context.MODE_PRIVATE)
    ...
    static Observable<List<IconReplacement>> refresh(Context context) {
